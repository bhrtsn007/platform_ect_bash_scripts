...
clearing_rollcage() {
    ...
    then
        ...
        if [ "$sr_id_present" != true ]; then
            sr_id_present=true
            echo "sr_id_present is not present previously marking it TRUE"
        fi
        echo ""
        ...
        then
            ...
            if [ "$data_lpn_attached" == 0 ] || [ ! -n "$data_lpn_attached" ]
            then
            echo "No LPN Found but SRID attached, some issue is there. Contact GOR"
            else
...
clearing_parent_rollcage() {
    ...
    then
        ...
        then
            if [ "$sr_id_present" == true ]; then
                sr_id_present=false
                sudo -u postgres psql -U postgres -d resources -c "update gresource set state = 'IN_USE' where id ='$id'"
                echo "One of child is having SR id, status should be IN_USE"
            fi
        elif [ "$state" == 'IN_USE' ]
        then
            if [ "$sr_id_present" == true ]; then
                sr_id_present=false
                echo "One of child is having SR id, status is already in IN_USE. ALL GOOD"
            fi
        else
            echo "Parent is not in IN_USE nor in READY_FOR_REUSE. Contact GOR "
        fi
    else
        srid_state=`sudo -u postgres psql -U postgres -d platform_srms -c "select state from service_request where id = '$srids_attached'" | head -3 | tail -1 | tr -d ' '`
        srid_status=`sudo -u postgres psql -U postgres -d platform_srms -c "select status from service_request where id = '$srids_attached'" | head -3 | tail -1 | tr -d ' '`
...
clearing_parent_rollcage $first
